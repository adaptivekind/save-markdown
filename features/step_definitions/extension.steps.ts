...
Before({ tags: '@extension' }, async function () {
  ...
  context = await chromium.launchPersistentContext('', {
    ...
    downloadsPath,
    headless: process.env.HEADED !== 'true' ? true : false,
  });
...
When('I add a new suggested rule with the following details:', async function (dataTable: DataTable) {
...
When('I navigate to the test page', async function () {
  ...
  const testPagePath = join(__dirname, '../../tests/e2e/fixtures/test-page.html');
  page = await context.newPage();
...
Then('a markdown file should be created', async function () {
  ...
  assert(downloadedFilename.includes('e2e-downloads'), 'Filename should contain e2e-downloads path');
...
Then('the markdown content should contain:', async function (dataTable: DataTable) {
  assert(downloadedFilename, 'Downloaded filename should be available');
